...
class Thread_dal extends Model
{
  ...
  {
    return $this->db->query("SELECT content, original_content, user_id, created FROM comments WHERE " .
  ...
  {
    $this->db->query("UPDATE comments SET original_content = ?, content = ? WHERE comment_id = ? " .
                     "AND user_id = ?", array($content, $processed, $comment_id, $user_id));
