...
$title_attr = $use_random_title ?
  ' class="changeling" title="'. $title->username .'"' : '';
$title = $use_random_title ? htmlspecialchars($title->title_text) : 'Threads';
...
?>
<div id="main-title" <?php echo $title_attr; ?>>
  <h3>
    <?php echo $title ?>
  </h3>
</div>
...
$display = $this->session->userdata('comments_shown') == false ?
     50 : $this->session->userdata('comments_shown');

if (isset($thread_result)) {
foreach($thread_result->result() as $row) {

  $alt = (!isset($alt) || $alt === false ? true : false);

  $link_text = '/thread/' . $row->thread_id . '/' .
    url_title(htmlentities($row->subject), 'dash', TRUE);
  ...
  if ($last_pagenumber > 1) {
    ...
    if ($last_pagenumber > 8) {
      $printpages .= ' <a href="'.$link_text.'/p/'.$last_page .
        '">...</a> <a href="'.$link_text.'/p/'.$last_page.'">' .
        $last_pagenumber.'</a>';
    }
  }
